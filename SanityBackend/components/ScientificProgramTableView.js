...
export default function ScientificProgramTableView() {
  ...
  const fetchDownloads = async () => {
    try {
      setLoading(true)
      const query = `*[_type == "scientificProgramDownload"] | order(downloadTimestamp desc) {
        _id,
        fullName,
        email,
        phone,
  ...
  const exportCSV = () => {
    ...
    a.href = url
    a.download = `scientific-program-downloads-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  ...
  if (loading) {
    return React.createElement(Card, { padding: 4 },
      React.createElement(Text, null, 'Loading scientific program downloads...')
    )
  }
        ...
        React.createElement(Flex, { align: 'center', justify: 'space-between' },
          React.createElement(Stack, { space: 2 },
            React.createElement(Text, { size: 4, weight: 'bold' },
              `📊 Scientific Program Downloads (${downloads.length})`
            )
          ),
        ...
        downloads.length === 0
          ? React.createElement(Card, { padding: 4, tone: 'transparent' },
              React.createElement(Text, { align: 'center', muted: true },
                'No Scientific Program downloads yet'
              )
            )

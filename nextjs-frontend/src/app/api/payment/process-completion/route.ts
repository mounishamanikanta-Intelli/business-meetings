...
const sanityClient = createClient({
  projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || '7qojy7w2',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const registrationQuery = `*[_type == "conferenceRegistration" && (registrationId == $registrationId ||
    paypalOrderId == $registrationId
    )][0] {
      _id,
      registrationId,
      personalDetails,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const statusQuery = `*[_type == "conferenceRegistration" && ( registrationId == $registrationId ||
    paypalOrderId == $registrationId
    )][0] {
      registrationId,
      paymentStatus,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const registrationQuery = `*[_type == "conferenceRegistration" && (registrationId == $registrationId ||
    paypalOrderId == $registrationId)][0]`;
    const registrationData = await sanityClient.fetch(registrationQuery, { registrationId });

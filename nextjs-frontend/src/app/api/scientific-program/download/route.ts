...
export async function GET(request: NextRequest) {
  try {
    ...
    const REDIS_KEY = 'intelli:'+process.env.NEXT_PUBLIC_BASE_URL+':brochure-download';

    console.log('📄 Processing scientific program brochure download request...');
    console.log('Download ID:', downloadId);

    // Fetch active brochure settings - prioritize singleton document
    const scientificProgramQuery = `*[_type == "scientificProgramSettings" && _id == "scientificProgramSettings" && active == true][0] {
      _id,
      title,
      description,
      pdfFile {
    ...
    const scientificProgramSettings = await client.fetch(scientificProgramQuery);

    if (!scientificProgramSettings || !scientificProgramSettings.active) {
    ...
    if (!scientificProgramSettings.pdfFile || !scientificProgramSettings.pdfFile.asset) {
      return NextResponse.json(
    ...
    if (downloadId) {
      const downloadRecord = await client.fetch(
        `*[_type == "scientificProgramDownload" && _id == $downloadId][0]`,
        { downloadId }
      );
    ...
    const pdfAsset = scientificProgramSettings.pdfFile.asset;
    ...
    return NextResponse.json({
      ...
      title: scientificProgramSettings.title,
      description: scientificProgramSettings.description,
    }, { headers: corsHeaders });

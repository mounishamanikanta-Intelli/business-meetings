...
const registrationSchema = z.object({
  ...
  // Participants
  numberOfAccompanyingPersons: z.number().min(0),
  ...
  accompanyingPersonsCost: z.number().min(0),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isFreeRegistration) {
      console.log('💳 Paid registration — stable ID assigned:', registrationId);
    }

    // Unified duplicate-check for both free and paid registrations
    {
      let attempts = 0;
      const maxAttempts = 5;
      ...
      while (attempts < maxAttempts) {
        try {
          const existingRegistration = await writeClient.fetch(
            `*[_type == "conferenceRegistration" && (registrationId == $registrationId || paypalOrderId == $registrationId || razorpayOrderId == $registrationId)][0]{ _id }`,
            { registrationId }
          );
    ...
    const registrationRecord = {
      ...
      numberOfAccompanyingPersons: registrationData.numberOfAccompanyingPersons,
      checkInDate: registrationData.checkInDate,
      ...
      pricing: {
        ...
        accompanyingPersonsCost: registrationData.accompanyingPersonsCost || 0,
      },
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔍 Searching for registration with ID:', registrationId);

    // Search by registrationId, paypalOrderId, or razorpayOrderId
    const registration = await client.fetch(
      `*[_type == "conferenceRegistration" && (registrationId == $registrationId || paypalOrderId == $registrationId || razorpayOrderId == $registrationId)][0]{
        _id,
        registrationId,
        paypalOrderId,
        personalDetails,
    ...
    if (registration) {
      ...
      console.log('   PayPal Order ID:', registration.paypalOrderId);
      console.log('   Customer Name:', registration.personalDetails?.firstName, registration.personalDetails?.lastName);

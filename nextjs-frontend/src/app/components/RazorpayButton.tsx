 ...
 *
 * UPDATED: Now saves registration to Sanity BEFORE creating payment order
 */
...
const RazorpayButton: React.FC<RazorpayButtonProps> = ({
  ...
  const handlePayment = async () => {
    ...
    try {
      // ============================================
      // STEP 1: Save registration to Sanity FIRST
      // ============================================
      console.log('💾 Saving registration before payment...');

      const saveRegistrationResponse = await fetch('/api/registration/save-before-payment', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          personalDetails: {
            title: registrationData.title || 'Dr.',
            firstName: registrationData.firstName,
            lastName: registrationData.lastName,
            email: registrationData.email,
            phoneNumber: registrationData.phoneNumber,
            country: registrationData.country,
            fullPostalAddress: registrationData.fullPostalAddress || registrationData.address
          },
          selectedRegistration: registrationData.selectedRegistration,
          selectedRegistrationName: registrationData.selectedRegistrationName,
          ...
          numberOfParticipants: registrationData.numberOfParticipants || 1,
          pricing: {
            totalPrice: amount,
            currency: currency,
            breakdown: registrationData.pricingBreakdown || {}
          }
        }),
      });

      if (!saveRegistrationResponse.ok) {
        const saveError = await saveRegistrationResponse.json();
        throw new Error(saveError.error || 'Failed to save registration');
      }

      const saveResult = await saveRegistrationResponse.json();
      console.log('✅ Registration saved successfully:', saveResult);

      // ============================================
      // STEP 2: Now create Razorpay order
      // ============================================
      ...
      const options = {
        ...
        name: 'International Gynaecology Conferences',
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            const finalRegistrationId = response.razorpay_order_id;
            console.log('🔷 Using Razorpay order ID as final registration ID:', finalRegistrationId);
            console.log('🔷 Old temp registration ID was:', currentRegistrationId);

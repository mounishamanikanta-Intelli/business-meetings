...
import React, { useState, memo, useRef, useEffect, use } from 'react';
...
const HeaderClient = memo(function HeaderClient({ siteSettings }: HeaderClientProps) {
  ...
  const [conferencesGallery, setConferencesGallery] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchConferencesGallery();
  }, []); // Empty dependency array - run once on mount

  const fetchConferencesGallery = async () => {
    try {
      const response = await fetch('/api/past-conference-gallery-visibility');
      const data = await response.json();

      console.log('🔍 HeaderClient: Past conferences Gallery visibility data:', data.showConferenceGallery);
      setConferencesGallery(data.showConferenceGallery);
      console.log('✅ HeaderClient: Updated showConferencesGallery to:', data.showConferenceGallery);
    } catch (error) {
      console.error('❌ HeaderClient: Error fetching conferences gallery visibility:', error);
    }
  };

  useEffect(() => {
    console.log('Conferences Gallery:', conferencesGallery);
  }, [conferencesGallery]);
